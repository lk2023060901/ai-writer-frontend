...
export const Stack = styled(Box).withConfig({
  shouldForwardProp: (
    prop: string,
    defaultValidatorFn: (prop: string) => boolean
  ) => {
    const blocked = prop === 'justifyContent' || prop === 'alignItems' || prop === 'flexDirection';
    return !blocked && (typeof defaultValidatorFn === 'function' ? defaultValidatorFn(prop) : true);
  }
})<StackProps>`
  display: flex;
